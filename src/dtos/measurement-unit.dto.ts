...
import {
  ...
  MeasurementUnitType,
} from "../infra/entities/measurement-unit.entity";
...
export type MeasurementUnitQueryDto = z.infer<
  typeof measurementUnitQuerySchema
>["query"];
...
export const CreateMeasurementUnitBodySchema = registry.register(
  ...
  z.object({
    name: z
      .string()
      .min(1, "Measurement unit name is required")
      .openapi({ example: "Kilogram" }),
    symbol: z
      .enum(MeasurementUnitSymbol, {
        message: "Invalid measurement unit symbol",
      })
      .openapi({ example: MeasurementUnitSymbol.KG }),
    type: z
      .enum(MeasurementUnitType, { message: "Invalid measurement unit type" })
      .openapi({ example: MeasurementUnitType.MASS }),
  })
);
...
export type CreateMeasurementUnitDto = z.infer<
...
export const UpdateMeasurementUnitBodySchema = registry.register(
  ...
  z.object({
    name: z
      .string()
      .min(1, "Measurement unit name cannot be empty")
      .optional()
      .openapi({ example: "Meter" }),
    symbol: z
      .enum(MeasurementUnitSymbol, {
        message: "Invalid measurement unit symbol",
      })
      .optional()
      .openapi({ example: MeasurementUnitSymbol.M }),
    type: z
      .enum(MeasurementUnitType, { message: "Invalid measurement unit type" })
      .optional()
      .openapi({ example: MeasurementUnitType.LENGTH }),
  })
);
...
export type UpdateMeasurementUnitDto = z.infer<
  typeof updateMeasurementUnitSchema
>["body"];
...
export function toMeasurementUnitResponseDto(
