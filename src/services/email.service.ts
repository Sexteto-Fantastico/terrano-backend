...
export const createEmailSender = (transporter: Transporter, emailFrom: string) => {
    return async (options: SendEmailOptions): Promise<void> => {
        ...
        if (!emailFrom) {
            throw new EmailConfigurationError("Sender email (EMAIL_FROM) is missing.");
        }
        ...
        try {
            ...
            throw new EmailSendError(`Could not send email to ${options.to}. Please try again later.`);
        }
    };
};
