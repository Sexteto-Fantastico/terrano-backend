...
): Promise<[DepartmentResponseDto[], number]> {
  const [allDepartments, total] =
    await DepartmentRepository.getAllDepartments(filters);
...
async function updateDepartment(
  ...
  if (data.costCenterCode !== undefined)
    existingDepartment.cost_center_code = data.costCenterCode;
